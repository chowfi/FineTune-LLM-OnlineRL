...
### 3e. Local web play (human vs LLM or MuZero)
- **Description:** `web/` + `scripts/serve_xiangqi_play.py` — FastAPI server and static board UI serving one of two engines, selected at launch (`--engine {llm,muzero}` / `XIANGQI_PLAY_ENGINE`). **LLM mode** (default): human plays Red; LoRA checkpoint (e.g. `checkpoints/xiangqi_grpo_v2/ep_40`) plays Black via greedy legal-move logprob scoring on a flipped board. **MuZero mode** (2026-07-06): human plays either color against the canonical checkpoint (`checkpoints/muzero_xiangqi/latest.pt`, `--ckpt` to override) via `web/server/muzero_player.py` (gate-strength search: `canonical_root` → 800-sim noiseless MCTS → `absolute_visits` → argmax, CPU by default) and `web/server/muzero_session.py` (runs on `muzero.env.XiangqiEnv` with hopeless-cp adjudication disabled; snapshot contract mirrors the LLM session's). MuZero mode never imports the 7B stack (`build_pikafish` lives in the light `web/server/pikafish_setup.py`; heavy imports are lazy inside `lifespan`'s LLM branch). Pikafish validates all moves in both modes. Session tests: `uv run pytest web/tests` (FakeEvaluator + tiny net, no engine/GPU needed). Spec: `docs/superpowers/specs/2026-07-06-muzero-web-play-design.md`. See `web/README.md`, `docs/logs/2026-05-28-log-xiangqi-web-play-ui.md`, `docs/logs/2026-07-06-log-muzero-web-play.md`.
