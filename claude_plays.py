...
PIECE_SYMBOL = {1:"G", 2:"A", 3:"A", 4:"E", 5:"E",
                6:"H", 7:"H", 8:"R", 9:"R",
                10:"C", 11:"C", 12:"S", 13:"S", 14:"S", 15:"S", 16:"S"}
PIECE_NAME   = {1:"General", 2:"Advisor", 3:"Advisor", 4:"Elephant", 5:"Elephant",
                6:"Horse", 7:"Horse", 8:"Chariot", 9:"Chariot",
                10:"Cannon", 11:"Cannon",
                12:"Soldier", 13:"Soldier", 14:"Soldier", 15:"Soldier", 16:"Soldier"}
PIECE_VALUE  = {1:100, 2:2, 3:2, 4:2, 5:2, 6:4, 7:4, 8:9, 9:9,
...
def col_letter(c): return "abcdefghi"[c]
...
def show_position(st):
    ...
    turn_num = sum(1 for i, a in enumerate(actions) if i % 2 == 0) + 1  # rough ally turn count

    print(f"\n{'='*60}")
    print(f"  Game {game_num}  |  Move {len(actions)+1}  |  Net reward so far: {total_reward:+.0f}")
    print(f"{'='*60}")
    ...
    print(f"  Run:  python claude_plays.py <idx>   to play a move")
    env.close()
...
def play_move(st, idx):
    ...
    if not done:
        ...
        if enemy_action is not None:
            _, pid_e, fp_e, tp_e, _ = list(filter(
                lambda m: m[0] == enemy_action,
                [(int(a), *action_space_to_move(a), "") for a in np.nonzero(uw2.enemy_actions)[0]]
            ))[0] if False else (enemy_action, *action_space_to_move(enemy_action), "")
            ...
            cap_e = f"  ★ captures your {PIECE_NAME.get(target_e,'?')}" if target_e > 0 else ""
    ...
    uw3 = env2.unwrapped
    print(f"\n{'='*60}")
    print(f"  Game {st['game_num']}  |  Move {len(actions)+1}  |  Net reward: {total_reward:+.0f}")
    print(f"{'='*60}")
